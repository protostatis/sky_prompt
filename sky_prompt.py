...
"""
        ...
        def test_resolve_pyreplab_command_ignores_sibling_dev_checkout(self) -> None:
            with tempfile.TemporaryDirectory() as temp_dir:
                root = Path(temp_dir)
                project_dir = root / "workspace"
                project_dir.mkdir()
                sibling_dir = root / "pyrepl"
                sibling_dir.mkdir()
                sibling_binary = sibling_dir / "pyreplab"
                sibling_binary.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
                sibling_binary.chmod(0o755)
